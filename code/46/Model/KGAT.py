...
class KGAT(object):
    ...
    def _build_weights(self):
        ...
        if self.pretrain_data is None:  # 每一行是一个embedding向量
    ...
    def _build_loss_phase_I(self):
        ...
        # BPR loss see paper 3.4 Optimization
    ...
    def _create_bi_interaction_embed(self):
        ...
        A = self.A_in  # A.shape = (184166, 184166)
        ...
        # ego_embeddings  ->  (10) e_t^{l-1}
        # A_fold_hat      ->  π(h,r,t)
        # side_embeddings ->  e_Nh^{l-1}
        # sum_embeddings  ->  (8) upper half
        # bi_embeddings   ->  (8) Bottom half


        for k in range(0, self.n_layers):
            # A_hat_drop = tf.nn.dropout(A_hat, 1 - self.node_dropout[k], [self.n_users + self.n_items, 1])
            temp_embed = []
            for f in range(self.n_fold):  # 矩阵的分块乘法, 然后合并
        ...
        all_embeddings = tf.concat(all_embeddings, 1)  # [(184166, 64), (184166, 64), (184166, 32), (184166, 16)]  ->  (184166, 176)
    ...
    def _split_A_hat(self, X):  # 根据行分割为100个小的tensorflow稀疏矩阵
        A_fold_hat = []
    ...
    def _create_attentive_A_out(self):
        ...
        # self.A_in.shape   ->   (184166, 184166)
        A = tf.sparse.softmax(tf.SparseTensor(indices, self.A_values, self.A_in.shape))
        # tf.SparseTensor  ->  https://zhuanlan.zhihu.com/p/94035220
        #
        ...
        #
        #
        # tf.sparse.softmax   first dim as N-batch
        # https://www.tensorflow.org/api_docs/python/tf/sparse/softmax

        return A

    def _generate_transE_score(self, h, t, r):  # 这不就是transR, 然后t和h+r相乘吗??
        embeddings = tf.concat([self.weights['user_embed'], self.weights['entity_embed']], axis=0)
        embeddings = tf.expand_dims(embeddings, 1)
        ...
        kg_score = tf.reduce_sum(tf.multiply(t_e, tf.tanh(h_e + r_e)), 1)  # this is pi (4)
